# Cases — track / sound / desk

Working cases for the ameobius desk. Sound-closed rows also live in `references/sound_corpus.md`.

## Coldwave spine (CW)

| ID | Track | Score | BPM | Status | File |
|---|---|---|---|---|---|
| CW-001 | ПОСЛЕДНЕЕ ОКНО | — | — | sound **closed** | sound corpus + source git |
| CW-002 / CW-012 | ТРИСТА СОРОК СЕМЬ | 8.4 | 112 | sound **closed** (alias) | sound corpus |
| CW-003 | РАЗМОРОЗКА | 8.3 | 108 | desk gen | [CW-003-razmorozka.md](./CW-003-razmorozka.md) |
| CW-004 | ЧУЖОЙ ЭТАЖ | 8.1 | 104 | desk gen | [CW-004-chuzhoj-etazh.md](./CW-004-chuzhoj-etazh.md) |
| CW-005 | СЧЁТЧИК | 8.2 | 100 | desk gen | [CW-005-schetchik.md](./CW-005-schetchik.md) |
| CW-006 | ВТОРАЯ СМЕНА | 8.3 | 98 | desk gen | [CW-006-vtoraya-smena.md](./CW-006-vtoraya-smena.md) |
| CW-007 | КРУГЛОСУТОЧНЫЙ | 8.2 | 96 | desk gen | [CW-007-kruglosutochnyj.md](./CW-007-kruglosutochnyj.md) |
| CW-008 | ПО ТУ СТОРОНУ КАССЫ | 8.2 | 94 | desk gen (♀ one-off) | [CW-008-po-tu-storonu-kassy.md](./CW-008-po-tu-storonu-kassy.md) |
| CW-009 | ДНЕВНОЙ СВЕТ | 8.2 | 104 | desk gen | [CW-009-dnevnoj-svet.md](./CW-009-dnevnoj-svet.md) |
| CW-010 | ОТТЕПЕЛЬ | 8.2 | 100 | desk gen | [CW-010-ottepel.md](./CW-010-ottepel.md) |
| CW-011 | ТИШИНА 2.0 | — | — | sound **closed** | sound corpus |
| CW-013 | ЧУЖОЙ ЭТАЖ (sound) | — | — | sound **closed** | sound corpus |
| CW-014 | СЧЁТЧИК ИДЁТ (sound) | — | — | sound **closed** | sound corpus |

## EN / IND lane

| ID | Track | Score | BPM | Status | File |
|---|---|---|---|---|---|
| EN-001 | RECEIPT STUCK | 8.1 | 118 | desk gen | [EN-001-receipt-stuck.md](./EN-001-receipt-stuck.md) |
| EN-002 | BUS WINDOW | 8.2 | 112 | desk gen | [EN-002-bus-window.md](./EN-002-bus-window.md) |
| EN-003 | COIN JAR | 8.2 | 108 | desk gen | [EN-003-coin-jar.md](./EN-003-coin-jar.md) |
| EN-004 | LAUNDROMAT | 8.1 | 104 | desk gen | [EN-004-laundromat.md](./EN-004-laundromat.md) |
| EN-005 | PARKING TICKET | 8.1 | 100 | desk gen | [EN-005-parking-ticket.md](./EN-005-parking-ticket.md) |
| EN-006 | VENDING SLOT | 8.1 | 96 | desk gen | [EN-006-vending-slot.md](./EN-006-vending-slot.md) |
| EN-007 | LOST GLOVE | 8.1 | 92 | desk gen | [EN-007-lost-glove.md](./EN-007-lost-glove.md) |
| EN-008 | SPARE KEY | 8.1 | 88 | desk gen | [EN-008-spare-key.md](./EN-008-spare-key.md) |
| EN-009 | EMPTY CHARGER | 8.1 | 84 | desk gen | [EN-009-empty-charger.md](./EN-009-empty-charger.md) |
| EN-010 | SECOND CUP | 8.1 | 80 | desk gen | [EN-010-second-cup.md](./EN-010-second-cup.md) |
| IND-001 | THIRD PASS | 8.4 | 140 | desk gen · sound pending | [IND-001-third-pass.md](./IND-001-third-pass.md) |
| IND-002 | KILL THE CREST | 8.3 | 134 | desk gen · sound pending | [IND-002-kill-the-crest.md](./IND-002-kill-the-crest.md) |

## Series lessons (CW)

- Pre-chorus **required**; chorus **×2** back-to-back (CW-003)
- No Mm-intro — plot intro only (CW-004)
- INST before bridge: two-phase preferred; plan B = 4-bar single (CW-004)
- No timestamp gimmick after CW-005
- Style = tag format + `professional studio recording` (CW-005)
- Anti-wail tags on intro/INST
- Default coldwave EQ: LS −2.5@100 · HS +4@3k · HS +4@10k · lim −0.5 dBTP
- mud_2k: bell −1.5@300 (CW-002)
- ≥2 Style slots change per track from CW-010 rule; core locked
- Male husky canon default; POV/gender break only spot (CW-008 lesson)
- Speak-aloud naturalness + premise logic pass (CW-009)
- Register move after earned warm day: thaw night, not spring-pop brighten (CW-010)

## EN / IND lessons

- Card note `Lang: EN`; craft micro-scaffold on card (EN-001..010, IND-001/002)
- Do **not** auto-port coldwave EQ into IND gens
- Anti-slop banks: therapy outro, pop-AI glue, cheap rhyme pairs, industrial cyber stamps
- YOU must act once (EN-001+)
- Outro = fact or action — no comfort lexicon
- IND fail_modes prefix `IND-*` in sound corpus
- IND-001→002 diptych: procedure pass → crest/kill peak (closed on desk)
- EN residual ladder: receipt → bus → coin jar → laundry → fine → vending → glove → spare key → charger → second cup (block closed)
